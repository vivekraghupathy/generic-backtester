...
class SixtyDayHighStrategy(BaseStrategy):
    """
    60-Day High Breakout Strategy
    Target: Stocks breaking above their 60-day high on strong volume.
    Filter: Stage 2 trend (Price > SMA200) and Strong Close.
    Exit: 21-day EMA trailing stop or 8% hard stop.
    """
    def __init__(self, name="60DayHigh", sma_period=200, volume_surge=1.5, stop_loss_pct=0.08, trailing_ema=21, close_proximity=0.01, lookback=60):
        super().__init__(name)
    ...
    def calculate_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        print(f" -> SMA{self.sma_period}, {self.lookback}-Day High, VolEMA21, EMA{self.trailing_ema}...")
        groups = df.groupby('Ticker')
        
        # 1. N-Day High (Highest High of last X days, excluding current day)
        df['RecentHigh'] = groups['High'].transform(lambda x: x.shift(1).rolling(window=self.lookback).max())
    ...
    def generate_signals(self, day_data: pd.DataFrame) -> pd.DataFrame:
        ...
        valid = day_data['RecentHigh'].notna()
    ...
    def calculate_entry_price(self, row: pd.Series, cash: float, total_max_positions: int, current_position_count: int) -> dict:
        ...
        entry_price = max(row['Open'], row['RecentHigh'] + 0.05)
